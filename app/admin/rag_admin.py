...
async def rag_management_page(request: Request):
    ...
    try:
        ...
        collection_stats = {
            "common_knowledge": await _get_collection_stats(
                rag_system.common_knowledge_db
            ),
        ...
        for role, collection in rag_system.role_collections.items():
            collection_stats["role_collections"][role] = await _get_collection_stats(
                collection
            )
...
async def upload_knowledge_base(
    ...
    try:
        ...
        upload_dir = os.getenv("TEMP_UPLOAD_DIR", "./data/tmp")
        ...
        if target_collection == "common_knowledge":
            ...
            await add_role_specific_content(
...
async def add_business_data(
    ...
    try:
        if format_type == "json":
            ...
            business_data = {"content": data_content, "added_at": "manual_entry"}
...
async def populate_sample_data():
    ...
    try:
        ...
        for role, content in role_contents.items():
            await add_role_specific_content(
                role, content, {"source": "sample_data", "type": "guidelines"}
            )
