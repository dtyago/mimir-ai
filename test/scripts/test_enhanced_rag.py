...
def create_test_pdf():
    ...
    try:
        ...
        c.drawString(100, 730, "This document contains information about business analytics and gaming.")
        c.drawString(100, 710, "Gaming Analytics: Track player engagement, retention, and monetization.")
        c.drawString(100, 690, "Business Metrics: Revenue, user growth, and market performance indicators.")
        c.drawString(100, 670, "Leadership Strategy: Vision, partnerships, and portfolio management.")
        c.drawString(100, 650, "Data Analysis: Cohort analysis, A/B testing, and predictive analytics.")
        c.save()
        pdf_buffer.seek(0)
        return pdf_buffer
    except ImportError:
...
def test_enhanced_rag_system(base_url="http://localhost:8000"):
    ...
    try:
        ...
        response = session.post(f"{base_url}/admin/rag/populate_sample_data", timeout=30)
        if response.status_code == 200:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"   ✅ Common Knowledge: {status_data['common_knowledge']['document_count']} docs")
            print(f"   ✅ Data Mart: {status_data['data_mart']['document_count']} docs")
            print(f"   ✅ Role Collections: {len(status_data['role_collections'])} collections")
    ...
    roles_to_test = ["Analyst-Gaming", "Analyst-Non-Gaming", "Leadership-Gaming", "Leadership-Non-Gaming"]
    auth_tokens = {}
    ...
    for i, role in enumerate(roles_to_test, 1):
        ...
        try:
            ...
            if test_image:
                files = {
                    'email': (None, f'test_{role.lower().replace("-", "_")}@example.com'),
                    'name': (None, f'Test User {role}'),
                    'role': (None, role),
                    'file': ('login-test.jpg', test_image, 'image/jpeg')
                }
                response = session.post(f"{base_url}/admin/register_user", files=files, timeout=30)
                ...
                if response.status_code == 200:
                    ...
                    login_response = session.post(f"{base_url}/user/login", files=files, timeout=30)
    ...
    for role, token in auth_tokens.items():
        ...
        for query_test in test_queries:
            try:
                ...
                response = session.post(f"{base_url}/user/chat", 
                                      json=chat_data, headers=headers, timeout=45)
                ...
                if response.status_code == 200:
                    ...
                    print(f"     ✅ {query_test['description']}: Enhanced RAG = {enhanced_rag_used}")
                    print(f"        📁 Data Sources: {', '.join(data_sources_used)}")
                    print(f"        🤖 Response Preview: {ai_response[:100]}...")
                    
                    # Check if expected context appears in response
                    context_found = any(ctx.lower() in ai_response.lower() 
                                      for ctx in query_test["expected_context"])
                    
                    test_results.append((f"Enhanced Chat - {role} - {query_test['description']}", True))
                    
                else:
                    print(f"     ❌ {query_test['description']} failed: {response.status_code}")
                    test_results.append((f"Enhanced Chat - {role} - {query_test['description']}", False))
    ...
    for role, token in auth_tokens.items():
        ...
        try:
            ...
            response = session.post(f"{base_url}/user/chat/data-sources", headers=headers, timeout=10)
            ...
            if response.status_code == 200:
                ...
                source_names = [src['name'] for src in available_sources if src['enabled']]
                print(f"      📁 Sources: {', '.join(source_names)}")
                
                test_results.append((f"Data Sources - {role}", True))
            else:
                print(f"   ❌ Data sources check failed for {role}: {response.status_code}")
    ...
    if auth_tokens:
        ...
        if sample_token:
            ...
            try:
                ...
                trad_response = session.post(f"{base_url}/user/chat", 
                                           json=traditional_data, headers=headers, timeout=30)
                ...
                enhanced_response = session.post(f"{base_url}/user/chat", 
                                                json=enhanced_data, headers=headers, timeout=30)
                
                if trad_response.status_code == 200 and enhanced_response.status_code == 200:
                    trad_data = trad_response.json()
                    enh_data = enhanced_response.json()
                    
                    print(f"   ✅ Traditional RAG: {len(trad_data.get('ai_response', ''))} chars")
                    print(f"   ✅ Enhanced RAG: {len(enh_data.get('ai_response', ''))} chars")
                    print(f"   📁 Enhanced used sources: {', '.join(enh_data.get('data_sources_used', []))}")
